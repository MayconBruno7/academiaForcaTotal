...
class Login extends ControllerMain
{
    ...
    public function index()
    {
        if (verificaSeUsuarioEstaLogado()) {
            return Redirect::page('sistema', ['msgSucesso' => "Usuário já logado!"]);
        } 
    ...
    public function signIn()
    {
        ...
        if (!$aUser) {
            $superUserCriado = $this->model->criaSuperUser();

            if ($superUserCriado) {
                return Redirect::page("/login", ['msgSucesso' => "Usuário administrador criado com sucesso! Email: administrador@gmail.com | Senha: admin"]);
            } else {
                return Redirect::page("/login", ['msgError' => "Usuário ou senha incorretos!."]);
            }
        }
